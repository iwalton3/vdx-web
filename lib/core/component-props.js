/**
 * How a declared prop's value gets INTO a component.
 *
 * Five things used to do this in defineComponent, each with its own copy of
 * the rules: the prototype setter, setProps, attributeChangedCallback, the
 * pre-construction pending values, and the attribute parse at connect. The
 * rules are all here, once, as a per-definition spec:
 *
...
export function createPropSpec(options) {
    ...
    // still unregistered (applyAttributeDirect's unowned-name path).
    //
