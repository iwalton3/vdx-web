...
export function toKeyedChild(result, key) {
    ...
    while (isWhen(result)) {
        const branch = result._condition ? result._thenValue : result._elseValue;
        result = typeof branch === 'function' ? branch() : branch;
        if (result == null || result === false) {
            result = EMPTY_WHEN_RESULT;  // keeps the key slot, renders nothing
            break;
        }
    }
