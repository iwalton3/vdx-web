...
// These are plain `Symbol()` (NOT Symbol.for) so external/attacker data can
// never obtain them - a plain object from JSON.parse can't carry the marker.
// HTML_MARKER is exported for framework-internal modules ONLY (the renderer
// stamps internally-built fragments with it); sharing it among trusted modules
// doesn't weaken the guarantee, which is that untrusted data can't forge it.
