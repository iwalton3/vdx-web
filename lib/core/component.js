                    ...
                    const instantiate = (templateResult) => {
                        ...
                        const valueGetters = values.map((initialValue, index) => {
                            if (typeof initialValue === 'function') {
                                // Functions (event handlers, renderItem, ...) get a
                                // wrapper whose identity is stable for the life of the
                                // component but which dispatches to the closure from
                                // the latest render. Passing the function through
                                // froze it at first render; wrapping it in a tracking
                                // getter instead would hand every child a new function
                                // identity on every render.
                                // `function`, not an arrow, so an explicit `this` at
                                // the call site still reaches the real handler.
