...
export function defineComponent(name, options) {
        ...
        _parseAttributes() {
            ...
            if (options.props) {
                for (const propName of Object.keys(options.props)) {
                    ...
                    if (pending) {
                        // Recorded under the name the TEMPLATE used, which is
                        // the kebab-case attribute form for a camelCase prop.
                        const attrForm = propAttrNames.get(propName) || propName;
                        if (pending.has(attrForm)) {
                            this.props[propName] = pending.get(attrForm);
                            continue;
                        }
                        if (pending.has(propName)) {
                            this.props[propName] = pending.get(propName);
                            continue;
                        }
                    }
    ...
    const createPropSetter = (propName) => {
        ...
        set(value) {
            // `undefined` means "not provided", not "the value undefined". The
            // first render already resolves it to the declared default
            // (_parseAttributes), so an update must agree - otherwise the
            // meaning of a prop depends on whether a render is the first one,
            // and `${maybeMissing}` silently blows past the default the second
            // time it evaluates. `null` stays an explicit null.
            if (value === undefined) value = options.props[propName];
