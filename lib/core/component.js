...
export function defineComponent(name, options) {
    ...
    // Guard: a method or computed named after a structural/attribute/event DOM
    // method would be bound onto the element and shadow the native one, breaking
    // the framework in ways that are miserable to debug. Throw at definition time
    // (covers both the class and options paths - both funnel into options.methods
    // / options.computed here). Props are only WARNED about: some (id, title,
    // hidden, ...) intentionally reflect to attributes, so a throw there would be
    // wrong - and props are not bound as callable methods anyway.
    ...
    // Guard: a method sharing a prop's name is a silent corruption bug. Props
    // are prototype accessors; methods are installed with `this[name] = fn` in
    // the constructor, which FIRES the prop's setter instead of defining the
    // method - so the method never lands AND the prop value is overwritten with
    // the function. There is no graceful winner to pick (unlike computed-vs-prop
    // below, where the computed is simply skipped), so reject at definition time.
    ...
    class Component extends HTMLElement {
        constructor() {
            ...
            if (this._pendingProps) {
                for (const [propName, value] of Object.entries(this._pendingProps)) {
                    ...
                    mirrorAttribute(this, propName, value);
                }
        ...
        connectedCallback() {
            ...
            // Capture light DOM children ONCE, before the first render, as the
            // live nodes themselves. Serialising through innerHTML and parsing
            // back made copies: a listener the page or a parent template had
            // put on a child was gone, and a nested component came back as a
            // new instance. The nodes are detached here and placed by the
            // template through props.children - a nested component takes its
            // reconnect path when that happens, not a fresh construction.
            if (!isReconnect && this.childNodes.length > 0) {
                ...
                // Whitespace and comments are dropped with the rest: the
                // render starts from an empty host either way.
        ...
        _parseAttributes() {
            // Values a template handed this element before its class was
            // registered. Highest precedence, because they are the only source
            // that still has the ${} value's TYPE - the attribute and the
            // inherited DOM property have both already coerced it to a string.
            // See lib/core/pending-props.js.
            const pending = takePendingProps(this);
            ...
            if (options.props) {
                for (const propName of Object.keys(options.props)) {
                    ...
                    if (pending && pending.has(propName)) {
                        // Recorded under the prop's own name: the renderer
                        // resolves the kebab attribute form before it decides
                        // ownership, so both sides agree on what the class owns.
                        // Mirrored like any other delivery, so tabindex="${0}"
                        // on a class that OWNS tabindex ends up as it would
                        // have had the class been registered at render.
                        const value = pending.get(propName);
                        this.props[propName] = value;
                        mirrorAttribute(this, propName, value);
                        continue;
                    }
    ...
    // render already resolves it to the declared default (_parseAttributes), so
    ...
    // null. Written ONCE: the setter and setProps are two entry points for the
    // same rule, and setProps (which the router uses) did not have it.
    ...
    // one - including a text mirror the renderer wrote while this element was
    // still unregistered (applyAttributeDirect's unowned-name path). Written
    // ONCE for the four places a prop value lands: the setter, setProps, the
    // pre-construction _pendingProps, and the template values taken at upgrade.
    ...
    const createPropSetter = (propName) => {
        ...
        set(value) {
            value = resolveIncoming(propName, value);
    ...
    Object.defineProperty(Component.prototype, 'setProps', {
        value: function setProps(newProps) {
            ...
            for (const [propName, rawValue] of Object.entries(newProps)) {
                ...
                const value = resolveIncoming(propName, rawValue);
