                    ...
                    const instantiate = (templateResult) => {
                        ...
                        const valueGetters = values.map((initialValue, index) => {
                            ...
                            const getter = () => {
                                ...
                                // A class must be handed over untouched: the wrapper
                                // calls through .apply(), which throws for a
                                // constructor, and would also hide .name, .prototype,
                                // statics and instanceof. Class prototypes are
                                // non-writable; ordinary functions' are writable.
                                const proto = Object.getOwnPropertyDescriptor(current, 'prototype');
                                if (proto && !proto.writable) return current;
                                if (!fnWrapper) {
