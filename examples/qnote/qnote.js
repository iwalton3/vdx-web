 ...
 */
import { defineComponent, html, when, flushSync, Component } from '../../lib/framework.js';
import { notify } from '../../lib/utils.js';
import { getRouter } from '../../lib/router.js';
import login from '../personal/auth/auth.js';
...
export class QnotePage extends Component {
    static stores = { login }
    ...
    // NOTE: `updated` is NOT a VDX lifecycle hook. The legacy options API silently
    // ignored this unknown key, so this route-param reactor never fired. It is kept
    // as a plain (never-called) method to preserve the original behavior. The route
    // does re-mount this component on param change (mounted() handles param loading),
    // so this dead handler was always redundant.
...
export default defineComponent('qnote-page', QnotePage);
